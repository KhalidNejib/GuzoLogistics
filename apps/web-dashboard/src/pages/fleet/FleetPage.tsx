...
export default function FleetPage() {
  ...
  return (
    <div className="mx-auto py-4 md:py-8 animate-fade-in space-y-4 md:space-y-6">
        ...
        <div>
          ...
          <div className="mt-1 flex items-center gap-2">
            <p className="text-sm font-medium text-slate-500">
              Managing verified pilots • Invite Key:
            </p>
            {merchantProfile?.fleetKey === 'RE-ONBOARD' ||
            (!merchantProfile?.fleetKey && riders.length === 0) ? (
              <button
              ...
              <button
                ...
                onClick={() => {
                  const key =
                    ...
                    (riders.length > 0 ? (riders[0] as any).fleetKey : '');
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2 border transition-all ${
                showPending
                  ...
                  : 'bg-white dark:bg-zinc-900 text-amber-600 border-amber-200 animate-pulse'
              }`}
            >
      ...
      {message && (
        <div
          className={`p-4 rounded-2xl flex items-center gap-3 border shadow-sm animate-in slide-in-from-top-2 ${message.type === 'success' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-red-50 text-red-700 border-red-100'}`}
        >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {pendingPilots.map((pilot) => (
                <div
                  key={pilot.user._id}
                  className="bg-white dark:bg-zinc-950 p-6 rounded-[32px] border-2 border-amber-100 dark:border-amber-900/30 shadow-xl shadow-amber-500/5 relative overflow-hidden group"
                >
                  <div className="flex gap-4 relative z-10">
                    ...
                    <div>
                      <h3 className="font-black text-slate-900 dark:text-white leading-tight">
                        {pilot.user?.fullName || 'Anonymous Pilot'}
                      </h3>
                      ...
                      <div className="flex gap-2 mt-2">
                        <Badge className="bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-slate-400 border-none text-[8px] font-black uppercase tracking-widest">
                          {pilot.vehicleType || 'Motorcycle'}
                        </Badge>
                        {pilot.licensePlate && (
                          <Badge className="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border-none text-[8px] font-black uppercase tracking-widest">
                            {pilot.licensePlate}
                          </Badge>
                        )}
                      </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-48 bg-slate-50 dark:bg-zinc-900 rounded-[32px] animate-pulse"
                />
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {filteredRiders.map((rider) => (
                <div
                  key={rider.riderId}
                  className="bg-white dark:bg-zinc-950 p-6 rounded-[32px] border border-slate-100 dark:border-zinc-900 shadow-sm hover:shadow-xl hover:border-blue-100 dark:hover:border-blue-900/30 transition-all group"
                >
                        ...
                        {editingId === rider.riderId ? (
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              onKeyDown={(e) =>
                                e.key === 'Enter' && handleEditConfirm(rider.riderId)
                              }
                          ...
                          <h3 className="font-black text-slate-900 dark:text-white truncate flex items-center gap-1.5 group/name">
                            ...
                            <Edit2
                              onClick={() => {
                                setEditingId(rider.riderId);
                                setEditName(rider.fullName);
                              }}
                              className="w-3 h-3 text-slate-300 opacity-0 group-hover/name:opacity-100 cursor-pointer"
                            />
                            {rider.disabled && (
                              <Badge className="bg-red-500 hover:bg-red-600 text-white text-[8px] font-black uppercase tracking-widest border-none ml-1">
                                Deactivated
                              </Badge>
                            )}
                          </h3>
                        )}
                        <p className="text-[10px] font-bold text-slate-400 mt-0.5 tracking-tighter">
                          ID: {rider.riderId.slice(-8).toUpperCase()}
                        </p>
                      </div>
                    </div>
                    {rider.cashHeld > 0 && (
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-6">
                    ...
                    <div className="p-3 bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-100/50 dark:border-blue-800/20">
                      ...
                      <p className="text-sm font-black text-blue-700 dark:text-blue-400">
                        {rider.avgDeliveryTimeMs
                          ? `${Math.round(rider.avgDeliveryTimeMs / 60000)}m`
                          : 'N/A'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t border-slate-50 dark:border-zinc-900 flex justify-between items-center">
                    <div className="flex flex-col">
                      ...
                      <p className="text-sm font-black text-emerald-600">
                        ETB {rider.totalRevenue.toLocaleString()}
                      </p>
                    </div>
                    <button
                      onClick={() => {
                        setDossierRider(rider);
                        setShowDossier(true);
                      }}
      ...
      {showDossier && (
        <RiderDossier
          ...
          onDelete={async (id) => {
            if (
              window.confirm(
                'Are you sure you want to permanently remove this pilot from the fleet? User connection credentials will be revoked immediately.'
              )
            ) {
