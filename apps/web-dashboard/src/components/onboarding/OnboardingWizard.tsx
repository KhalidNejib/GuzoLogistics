...
import {
  ...
  ChevronRight, ChevronLeft, Loader2, Truck,
  Star, Zap, Package,
} from 'lucide-react';
...
const ETHIOPIAN_CITIES = [
  ...
  'Bahir Dar', 'Adama', 'Jimma', 'Jijiga', 'Shashamane',
  'Bishoftu', 'Arba Minch', 'Hosaena', 'Wolaita Sodo', 'Dilla',
];

const STEPS = [
  { id: 1, label: 'Identity',  icon: Building2,   title: 'Your Business Identity',    sub: 'How customers and riders will know you' },
  { id: 2, label: 'Location',  icon: MapPin,       title: 'Service Territory',         sub: 'Where your fleet will operate' },
  { id: 3, label: 'Pricing',   icon: DollarSign,   title: 'Delivery Pricing',          sub: 'Set your base fare and per-km rate' },
  { id: 4, label: 'Launch',    icon: CheckCircle2, title: 'Ready for Takeoff',         sub: 'Review and activate your account' },
];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-blue-300 mt-2 font-medium">Set up your merchant account in 3 quick steps</p>
        </div>
            ...
            <div key={s.id} className="flex items-center flex-1">
              <div className="flex flex-col items-center">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-sm transition-all duration-300 ${
                  ...
                  step === s.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30 ring-4 ring-blue-500/20' :
                  'bg-white/10 text-white/40'
                }`}>
                  {step > s.id ? <CheckCircle2 className="w-5 h-5" /> : <s.icon className="w-4 h-4" />}
                </div>
                <span className={`text-[10px] font-bold mt-1.5 uppercase tracking-wider ${step === s.id ? 'text-blue-300' : 'text-white/30'}`}>{s.label}</span>
              </div>
              {i < STEPS.length - 1 && (
                <div className="flex-1 h-[2px] mx-3 mt-[-16px] rounded-full overflow-hidden bg-white/10">
                  <div className="h-full bg-blue-500 transition-all duration-500" style={{ width: step > s.id ? '100%' : '0%' }} />
                </div>
              )}
            </div>
          ))}
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl overflow-hidden">
          {/* Card Header */}
          <div className="h-2 bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600" style={{ width: `${progress + 25}%`, transition: 'width 0.4s ease' }} />
          ...
          <div className="p-8">
            ...
            {step === 1 && (
              <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                <div>
                  <label className="text-[11px] font-black uppercase tracking-wider text-slate-500 block mb-2">Business Name *</label>
                  <input
                    autoFocus
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[11px] font-black uppercase tracking-wider text-slate-500 block mb-2">Support Email</label>
                    <input
                      type="email"
                  ...
                  <div>
                    <label className="text-[11px] font-black uppercase tracking-wider text-slate-500 block mb-2">Phone Number</label>
                    <input
                      type="tel"
                ...
                <div>
                  <label className="text-[11px] font-black uppercase tracking-wider text-slate-500 block mb-2">Business Address</label>
                  <input
            ...
            {step === 2 && (
              <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                <div>
                  <label className="text-[11px] font-black uppercase tracking-wider text-slate-500 block mb-3">Select Your Service City *</label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {ETHIOPIAN_CITIES.map(city => (
                      ...
                      >
                        <MapPin className={`w-3.5 h-3.5 mb-1 ${data.serviceCity === city ? 'text-blue-500' : 'text-slate-400'}`} />
                        {city}
                      </button>
                    ))}
                  </div>
                ...
                {data.serviceCity && (
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center gap-3 animate-in fade-in duration-200">
                    ...
                    <p className="text-sm font-bold text-blue-700">
                      Your riders in <span className="underline">{data.serviceCity}</span> will automatically receive orders from your dispatch.
                    </p>
                  </div>
                )}
              </div>
            ...
            {step === 3 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <label className="text-[11px] font-black uppercase tracking-wider text-slate-500 block mb-2">Base Fare (ETB) *</label>
                    <div className="relative">
                      <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-black text-sm">ETB</span>
                      <input
                        type="number"
                        min={0}
                        value={data.baseFare}
                    ...
                    <p className="text-[10px] text-slate-400 font-medium mt-1.5">Flat fee charged per order</p>
                  </div>
                  <div>
                    <label className="text-[11px] font-black uppercase tracking-wider text-slate-500 block mb-2">Per KM Rate (ETB) *</label>
                    <div className="relative">
                      <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-black text-sm">ETB</span>
                      <input
                        type="number"
                    ...
                    <p className="text-[10px] text-slate-400 font-medium mt-1.5">Added per kilometer of distance</p>
                  </div>
                </div>

                {/* Live pricing preview */}
                <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-6 text-white">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Live Pricing Preview</p>
                  {[
                    { label: 'Short Delivery (2 km)', km: 2 },
                    { label: 'Medium Delivery (7 km)', km: 7 },
                    { label: 'Long Delivery (15 km)', km: 15 },
                  ].map(ex => (
                    <div key={ex.km} className="flex items-center justify-between py-2.5 border-b border-white/10 last:border-0">
                      <div>
                        <p className="text-sm font-bold">{ex.label}</p>
                        <p className="text-[10px] text-slate-400">Base + ({ex.km} km × ETB {data.perKmRate})</p>
                      </div>
              ...
              <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="bg-slate-50 rounded-2xl p-6 space-y-4">
                  {[
                    ...
                    { icon: DollarSign, label: 'Pricing', value: `ETB ${data.baseFare} base + ETB ${data.perKmRate}/km` },
                    ...(data.supportEmail ? [{ icon: Star, label: 'Support Email', value: data.supportEmail }] : []),
                  ].map(item => (
                ...
                <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-emerald-700 text-xs font-medium mt-1">
                      Click <strong>Launch My Business</strong> to activate your merchant account. You can change all these settings anytime from the Settings page.
                    </p>
                  </div>
                </div>
          ...
          <div className="px-8 pb-8 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {STEPS.map(s => (
                <div key={s.id} className={`rounded-full transition-all duration-300 ${step === s.id ? 'w-6 h-2 bg-blue-600' : 'w-2 h-2 bg-slate-200'}`} />
              ))}
            </div>
            ...
            {step < 4 ? (
              ...
              >
                {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Launching...</> : <><Package className="w-4 h-4" /> Launch My Business</>}
              </button>
            )}
          </div>
        </div>
